...
describe("The inViewport method", () => {
	beforeEach(() => {
		const $element = Cypress.$(
			`<button id="test" style="position: absolute">Test</button>`,
		);
	...
	describe("In regular mode", () => {
		...
		it("should throw when element is not in viewport", (done) => {
			...
			expectAssertionErrorOnFail(
				...
				"expected '<button#test>' to be visible inside viewport",
			);
		...
		it("should throw when invoked without the window argument", (done) => {
			...
			cy.window().then((w) => {
				expect(Cypress.$("#test")).to.be.inViewport(
	...
	describe("In negative mode", () => {
		...
		it("should throw when element is in viewport", (done) => {
			...
			expectAssertionErrorOnFail(
				...
				"expected '<button#test>' to not be visible inside viewport",
			);

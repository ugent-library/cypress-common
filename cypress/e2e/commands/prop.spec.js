describe('The prop command', () => {
  ...
  it('should return a property from an object', () => {
    cy.get('#someLink').as('some-link').prop('href').should('eq', 'https://www.google.com/abc')

    cy.get('@some-link').prop('innerText').should('eq', 'test')

    cy.get('@some-link').prop('outerHTML').should('eq', someLink)
  })

  it('should be possible to set a property on an object', () => {
    cy.get('#someLink').as('some-link').prop('title', 'some other title').prop('href', '/def')
    ...
    cy.get('@some-link').prop('title').should('eq', 'some other title')

    cy.get('@some-link').prop('href').should('eq', 'https://www.google.com/def')
  })
})
